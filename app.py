...
from flow_inference_utils import build_cfg, load_artifacts
...
# NEW: Import the random code generator
...
flush_penalty = st.number_input(
    "Branch Misprediction Penalty (flush cycles):", 0, 5, 2, key="flush_penalty_input"
)
...
if run:
    ...
    # --- Display Node and Adjacency Info --- (UNCHANGED)
    ...
    # FIXED: Remove 'seed' as kamada_kawai_layout doesn't support it (deterministic anyway)
    pos = nx.kamada_kawai_layout(G)

    # UPDATED: Larger figure for more space, smaller nodes/arrows for fit
    plt.figure(figsize=(12, 8))  # Increased from (7,5) for better visibility
    nx.draw(
        G,
        pos,
        with_labels=True,
        node_size=800,  # Reduced from 1800
        node_color="lightgreen",
        arrowsize=15,  # Slightly smaller arrows
        font_size=8,
        font_weight="bold",  # Smaller font to avoid crowding
        edge_color="gray",
        width=1.0,
    )  # Added: Thinner edges for clarity
    st.pyplot(plt)

    # --- Load LSTM model --- (UNCHANGED)
    model, tokenizer, meta = load_artifacts("cfg_lstm")

    # --- Run rule-based & LSTM flow --- (UNCHANGED)
    ...
    # --- Simulate both pipelines --- (UNCHANGED)
